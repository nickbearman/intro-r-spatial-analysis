...
date: "Tue 11th and Wed 12th Nov 2025"
---
...
*If you need to install RStudio, please check [here](https://nickbearman.github.io/installing-software/r-rstudio) or if you can't install it, an alternative option is to use [Posit.cloud](https://posit.cloud) to run RStudio in a web browser.*
...
*This data is downloaded from [https://data.police.uk/](https://data.police.uk/). If you want, feel free to download a more recent version and use that. Remember we need to download data for Merseyside Police for it to cover the same geographic area as the LSOA data, which we will be using later!*

*It is also worth mentioning that the latitude and longitude locations of Crime incidents published on this site always represent the approximate location of a crime — not the exact place that it happened. For more details, see [https://data.police.uk/about/#location-anonymisation](https://data.police.uk/about/#location-anonymisation).*

```{r message=FALSE, warning=FALSE, comment=NA}
#Read the data into a variable called crimes
crimes <- read.csv("http://nickbearman.com/files/police-uk-2025-08-merseyside-street.csv")
```
...
### Police API Example (optional exercise)

The crime data is available through an API as well, so instead of downloading a `csv` file, you can access the data programmatically. This is useful when data updates, although in this case it is worth noting that the Crime data is updated monthly, and can often take 2-3 months to be processed before it is available. 

If you have time, and want to experiment, see if you can access the Crime data via an API. 

How do you do this? I will let you figure this out. But as a starting point, try consulting a LLM of your choice (e.g. [Claude.ai](Claude.ai)) and ask something like: "How do I use Police API with R? https://data.police.uk/docs/".
...
This is the opportunity to use some of the techniques we have covered on your own data. If you don't have any data with you, you can download some sample data from the links in the slides. 

----

This practical was written using R 4.5.2 (2025-10-31), RStudio 2025.09.2+418 and `tmap` 4.2 by Dr. Nick Bearman (nick@nickbearman.com). It has also been tested in Posit Cloud, with R 4.5.1 (2025-06-13) and RStudio 2025.05.1. 
...
The latest version of the PDF is available from https://github.com/nickbearman/intro-r-spatial-analysis. This is v10.1 and this was created on `r format(Sys.time(), '%d %B %Y')`. 
