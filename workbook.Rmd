...
output:
  ...
  html_document:
    df_print: paged 
date: "Tue 1st and Wed 2nd February 2022"
---
...
- Under **Census Statistics** (scroll down) click **2011 Data catalogue**. 
...
The final line of code (starting `colnames`) actually updates the variable names. The square brackets are used to refer to specific elements - in this case, columns 5 to 23. 

*For example, `pop2011[1,]` will show the first row and `pop2011[,1]` will show the first column.*
...
The output isn't that elegent - it repeats the output (of length) across every column. So we can tidy this up, rename the columns and map the data.
...
This just tells us how many crimes there were in each LSOA - nothing about the rate. Try calculating the rate of the crimes per 10,000 people using the population data from earlier. *You will need to join the population data, `pop2011`.*
...
This practical was written using R 4.0.2 (2020-06-22) and RStudio 1.3.1093 by Dr. Nick Bearman (nick@geospatialtrainingsolutions.co.uk). It has also been tested in R 4.2.0 (2022-04-22) and RStudio 2022.02.2 (RStudio Cloud). 
