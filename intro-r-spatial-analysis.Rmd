...
- Navigate to http://www.neighbourhood.statistics.gov.uk. 
- Select the “Topics” link on the bottom left hand side of the page. 
...
#setup example text string which is the column name
  textstring <- "Age0to4pc"
#use substr to extract the 4th and 7th characters
  firstyear <- substr(textstring, 4, 4)
  lastyear <- substr(textstring, 7, 7)
#create title
...
An additional point - what happens when the years are two digits? How could we alter the code (or field names) to take this into account?
...
This practical was written using R 3.3.3 (2017-03-06) and RStudio 1.0.136 by Nick Bearman (nick@clearmapping.co.uk). 
