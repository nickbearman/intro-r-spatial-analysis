...
\textbf{\texttt{\%>\%}} a pipe, used by packages from the tidyverse
...
\textbf{aggregate()} group the specified data set by a column, applying a function to the values:\\\texttt{aggregate(x = LSOA\_crimes,\\
by = list(LSOA\_crimes\$lsoa21cd), FUN = length)}
...
\textbf{classIntervals()} calculate the class intervals for the specified data: \texttt{classIntervals(var, n = 6, style = "fisher")}
...
\textbf{download.file()} download a file from a URL: \texttt{download.file("http://www.nickbearman.com/files/
sthelens\_lsoa\_2021.zip","sthelens\_lsoa\_2021.zip")}
...
\textbf{fill.chart()} Add items to the map (e.g. 
histogram): \texttt{fill.chart = tm\_chart\_histogram()}

\textbf{fill.legend()} Add items to the legend (e.g. title): \texttt{fill.legend = tm\_legend(title = "Aged 0 to 4", size = 0.8)}

\textbf{fill.scale()} Specify the colours and classification: \texttt{fill.scale = tm\_scale\_intervals(values = "brewer.greens", n = 6, style = "jenks")}

\textbf{Fisher} \textit{Classification} method very similar to \textit{Jenks} 

\textbf{for () \{\}} begins a loop to make R repeat a command a set number of times:\\
\texttt{for (i in 1:length(mapvariables))}
...
\textbf{if () \{\} } begins an if loop to make R choose a specific command based on a criteria
...
\textbf{Layers} When you add data into a GIS each different file appears as a different \textit{layer}; this allows different datasets to be overlaid on one another
...
\textbf{project file} A file created by GIS to store the symbology, classification, layouts and a list of the data layers you are using. Project files do not store any spatial data. Examples include .qgz / .qgs (QGIS) and .aprx (ArcGIS Pro)

\textbf{Projected coordinate system} A coordinate system covering a specific, small area, usually uses meters and can be used for measuring distances (see also \textit{BNG}, \textit{eastings}, \textit{northings} and \textit{UTM})
...
\textbf{read.csv()} read in a \textit{csv} file from the specified path: \texttt{read.csv("police-uk-2025-08-merseyside-street.csv")}
...
\textbf{Spatial autocorrelation} A measure of how much the location of spatial data is correlated with itself, a value of 1 is a strong positive autocorrelation meaning data are spatially clustered with low values near each other and high values near each other, 0 is no spatial autocorrelation meaning data are distributed at random and space is not important in this data set, -1 is a strong negative correlation with low and high values as mixed as possible
...
\textbf{summarise()} an alternative to \textit{aggregate}, from the dplyr library

\textbf{T / TRUE} used to specify that we do want something: \texttt{legend.hist = T} means we want to show the legend histogram

\textbf{table()} count how many different values there are in the specified column, and output as a table: \texttt{table(crimes\_sf\_bng\$Crime.type)}

\textbf{Tabular data} Data laid out in rows and columns, as used in Excel (see also \textit{CSV})

\textbf{title.position} specify the location of the title on a map: \texttt{title.position = c(0.7, \q{top})}

\textbf{tmap} library used for creating maps in R
...
\textbf{tm\_symbols()} specify the symbology of a point layer on a map: \texttt{tm\_symbols(size = 0.1, col = "darkred", col\_alpha = 0.5)}

\textbf{tm\_title()} specify the title of a map: \texttt{tm\_title("Total Population of Liverpool, 2021")}
...
\textbf{Variables} How R stores data, either text, numbers or spatial data
